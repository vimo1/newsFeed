...
protocol RssFeedPresentableListener: class {
  var stories: [Story] { get }
  var rssFeeds: [RssFeedResponse] { get }
...
final class RssFeedViewController: UIViewController, RssFeedPresentable, RssFeedViewControllable, StoryViewCellListener {

  weak var listener: RssFeedPresentableListener?
  ...
  override func viewDidLoad() {
    super.viewDidLoad()
    shuffledStoryCellIds.append(RssFeedViewController.storyImageTopCellId)
  ...
  func shareStory(cell: UITableViewCell) {
    guard let indexPath = storiesTableView.indexPath(for: cell),
      let story = listener?.getStory(atIndexPath: indexPath) else { return }
...
extension RssFeedViewController: UITableViewDataSource, UITableViewDelegate {
  ...
  func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
    guard listener?.rssFeeds.count ?? 0 > section else {
      return 0
    }
    return listener?.rssFeeds[section].stories.count ?? 0
  }
  
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    
    guard let story = listener?.getStory(atIndexPath: indexPath) else { return UITableViewCell() }
    ...
    if story.urlToImage?.count ?? 0 > 0 {
      ...
      storyCellId = RssFeedViewController.storyNoImageCellId
    }
